...
#-- FUNCTION
#------------------------------------------------------------------------
def squeeze_and_nanify(x):

    if x.dtype == 'float32':
        index_replace = (x == default_fillvals['f4'])
    elif x.dtype == 'float64':
        index_replace = (x == default_fillvals['f8'])
    else:
        print('squeeze_and_nanify only valid for floats and doubles')
        exit(1)

    x[index_replace] = np.nan
