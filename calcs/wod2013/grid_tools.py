...
def generate_latlon_grid(nx,ny,lon0=0.,file_out=''):
    ...
    ds = xr.Dataset(
        {'latitude':xr.DataArray(latitude,
                                 dims=('latitude'),
                                 attrs={'units':'degrees_north',
                                        'long_name':'Latitude'}),
         'longitude':xr.DataArray(longitude,
                                  dims=('longitude'),
                                  attrs={'units':'degrees_east',
                                         'long_name':'Longitude'})})

    ds['xc'] = xr.DataArray(xc,dims=('latitude','longitude'),
                            attrs={'units':'degrees_east',
                                   'long_name':'longitude of cell centers'})

    ds['yc'] = xr.DataArray(yc,dims=('latitude','longitude'),
                            attrs={'units':'degrees_north',
                                   'long_name':'latitude of cell centers'})

    ds['xv'] = xr.DataArray(xv,dims=('latitude','longitude','nv'),
                            attrs={'units':'degrees_east',
                                   'long_name':'longitude of cell corners'})
    ds['yv'] = xr.DataArray(yv,dims=('latitude','longitude','nv'),
                            attrs={'units':'degrees_north',
                                   'long_name':'latitude of cell corners'})

    ds['area'] = xr.DataArray(area,dims=('latitude','longitude'),
                              attrs={'units':'m^2',
                                     'long_name':'area'})
